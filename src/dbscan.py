...
pcd_path = "../data/results/background_diff/case02/background_diff.pcd"
...
for i in range(max_label + 1):
    ...
    # クラスタごとに色を付与
    color = plt.get_cmap("tab20")(i / (max_label if max_label > 0 else 1))[:3]
    cluster.paint_uniform_color(color)
    o3d.io.write_point_cloud(f"../data/results/dbscan/each_cluster/case02/cluster_{i:02d}.pcd", cluster)



# 全体（色付き）の点群も保存
o3d.io.write_point_cloud("../data/results/dbscan/overall/case02/colored_clusters.pcd", pcd)

# --- クラスタ情報をJSONで保存 ---
with open("../data/results/dbscan/each_cluster/case02/cluster_info.json", "w") as f:
